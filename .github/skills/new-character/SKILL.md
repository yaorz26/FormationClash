...
- **`【】`或`[]`** 包裹 → 整局一次的主动技能（`ActiveSkill.once_per_game=True`）
- **`（）`或`()`** 包裹 → 普通主动技能（非整局一次）
- **无括号** → 被动效果
- 技能没有明确目标限制 → 默认可选任意存活角色（不受前后排/敌我限制），前后排规则默认 **只影响普通攻击**
...
- 只收录**已验证的、稳定的**模式，不收录实验性或一次性的实现
- 保持"核心原则"简洁，细节放入对应章节
- 检查清单和参考附表应随代码实际结构保持同步
- 每次更新后在 Git 提交信息中注明变更摘要
